...
class FilterController extends Controller
{
    ...
    public function filterResults(Request $request)
    {
        ...
        // Retrieve filtered data from the appropriate table
        $filteredResults = DB::table($tableName)
        ...
        // Organize the filtered data based on gender, residence, and education level
        $genderData = $filteredResults->groupBy('gender')->map->count();
        $residenceData = $filteredResults->groupBy('residence')->map->count();
        $educationLevelData = $filteredResults->groupBy('education_level')->map->count();

        // Pass the organized data to the view
        return view('filtered-results')->with([
            'genderData' => $genderData,
            'residenceData' => $residenceData,
            'educationLevelData' => $educationLevelData,
        ]);
